...
if __name__ == '__main__':
    ...
    with Pool(14) as p:
        ...
        for r in tqdm(p.imap(main, sparse), total=len(sparse)):
            res.append(r)
        pd.DataFrame.from_records(res, index=sparse).to_csv('../results/seg_prune_eval_sparse.csv')
        ...
        for r in tqdm(p.imap(main, dense), total=len(dense)):
            res.append(r)
        pd.DataFrame.from_records(res, index=dense).to_csv('../results/seg_prune_eval_dense.csv')
